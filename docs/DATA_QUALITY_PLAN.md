...
| # | 항목 | 증상 | 근거 |
|---|---|---|---|
| ~~D1~~ | ~~`sentiment.vkospi` 값 오류~~ | **철회.** VIX 대비 4.9배라 오염값으로 의심했으나, KOSPI 일별 종가로 실현변동성을 직접 계산하니 20일 **100.4%** / 60일 80.6% 였다(최근 일간 수익률에 -10.8%, +17.9%). 같은 계산에서 S&P500 은 13.3% 로 VIX 15.99 와 정합. **VKOSPI 78~82 는 정상값**이며, 남은 문제는 §1.2 S2 의 최신성뿐이다 | 실현변동성 계산 |
...
| ~~D5~~ | ~~VKOSPI 범위 가드 무력~~ | **철회.** 2026-06 유가 86.55 오통과는 실제 사건이나, 그 대응으로 배수 상한을 4.0 → 9.0 으로 넓힌 것은 옳은 판단이었다(실현변동성 검증 결과 참조). 가드를 다시 좁히면 진짜 값을 죽인다 | 실현변동성 계산 |
...
| VKOSPI | 4단계 폴백 전멸 | KRX OpenAPI **파생상품지수 엔드포인트** / Worker 경유 — 값은 정상, 소스만 사망 | 미해결 |
| stockMovers | pykrx (로그인 실패) | 사용자가 시크릿 등록 불가로 결정 → **무인증 폴백 강화 필요** | 미해결 |
...
### P3 — VKOSPI: 값이 아니라 소스가 문제다

당초 이 절은 "VIX 대비 배수 가드를 좁힌다"였으나 **폐기했다.** KOSPI 일별 종가로
실현변동성을 직접 계산한 결과 20일 **100.4%** / 60일 80.6% 였다(최근 일간 수익률에
-10.8%, +17.9%). 같은 계산에서 S&P500 은 13.3% 로 VIX 15.99 와 정합한다.
즉 **VKOSPI 78~82 는 정상값**이고, 배수를 좁히면 진짜 값을 죽인다. 코드의 교차검증
상한이 4.0 → 9.0 으로 넓혀진 이력이 바로 이 함정을 밟았던 흔적이다 — 좁히지 말 것.

남은 진짜 문제는 최신성이다. 폴백 4단계가 전멸해 as-of 가 2026-07-27 에 고착됐고,
필요한 조치는 가드가 아니라 **살아있는 소스 확보**다:
1. KRX OpenAPI 파생상품지수 엔드포인트 확인 (현재는 코스피 지수 계열만 조회)
2. investing.com 이 Actions 데이터센터 IP 에서 차단되는지 확인 → 차단이면 Worker 경유
3. 그래도 실패하면 값을 만들지 말고 `dataHealth` 에 stale 로 남긴다(현재 동작)
