...
- **발송 형태**: 슬롯마다 **① 차트 이미지(피드) + ② 전체 요약 텍스트** 로 보냅니다. 카카오는 한 통에 '이미지+긴 텍스트'를 같이 못 담고 텍스트는 200자 한도라, 요약이 길면 텍스트가 **여러 통으로 분할**됩니다(보통 이미지 1 + 요약 2~3통). 통 수를 줄이려면 `scripts/send_kakao_digest.py` 에서 Top 개수(`_movers_line` 의 `n`)를 줄이거나 일부 카테고리 블록을 빼면 됩니다.
