...
**모든 발송이 '카드 사진 한 장 + 피드 한 통'입니다** (기획 v3 — 정기 시황·주간 리포트·종목 알림·
급변 속보·서킷 발동/해제·테스트·장 마감이 전부 정사각 카드 1080×1080 을 받습니다. 카드 렌더나
이미지 업로드가 실패할 때만 슬롯 라인 차트 → 텍스트로 순서대로 내려갑니다):

- 슬롯별 **차트 이미지 1장** (당일 인트라데이 기준, 당일 데이터가 없으면 **7일 일봉으로 폴백**)
- 제목: `M/D(요일) H시 시황`
- 공통 내용 4종 — **증시**(코스피·S&P) / **환율**(달러-원·달러-엔) / **심리**(공포탐욕·VIX) / **원자재**(WTI·금·구리)
- `대시보드 보기` 버튼 (카카오는 버튼 2개·라벨 8자가 상한이라, 발송 종류별로 두 칸만 배정합니다 —
  시황=[대시보드 보기][히어로 지표 시세] / 종목 알림=[투자현황][종목] / 급변·서킷=[주식시장] 또는 [시장 지표])

| 구분 | 시각(KST) | 차트 (위 + 아래) |
|---|---|---|
| 평일 | **07시** | 미국 10년 국채 + 일본 10년 국채 |
| 평일 | **08시** | 달러-원 + 금 |
| 평일 | **09시** | S&P500 + 코스피 |
| 평일 | **10시 ~ 16시** | 코스피 + 달러-원 |
| 평일 | **17시** | 달러-원 + 금 |
...
| 평일 | **19시** | 은 + 구리 |
| 평일 | **20시** | 천연가스 + 밀 |
| 평일 | **21시** | 한국 10년 국채 + 미국 10년 국채 |
| 평일 | **22시** | 달러-원 + WTI |
| 주말 | **11시 · 17시** | 달러-원 + 금 *(본문은 평일과 동일)* |

*(위 표는 `scripts/send_kakao_digest.py` 의 `SLOT_CHARTS_*` 와 항상 일치해야 합니다 — 2026-07 감사에서
문서가 구버전 구성으로 남아 "받은 차트가 안내와 다르다"는 혼선을 유발해 동기화했습니다.)*
...
### ⑤ (선택) 친구에게 보내기 — 푸시 알림 받기

> ⚠ **현재 기본값은 꺼짐(`KAKAO_FRIENDS=0`)입니다** (2026-07 결정 — 토큰에 `friends` 스코프가
> 없어 매 발송이 403 → 무음 폴백만 반복했음). 아래 절차로 `friends` 동의 토큰을 발급해
> `KAKAO_REFRESH_TOKEN` 을 교체한 뒤, 저장소 **변수** `KAKAO_FRIENDS=1` 을 설정해야 켜집니다.
...
- **🔗 링크가 엉뚱한 사이트(예: `localhost…`)로 열릴 때**: 메시지의 이미지·버튼 링크 도메인은 카카오 앱에 **등록된 사이트 도메인**이어야 합니다. 미등록이면 카카오가 등록된(잘못된) 도메인으로 대체합니다.
  → 카카오 developers → 내 애플리케이션 → **앱 설정 → 플랫폼 → Web → 사이트 도메인** 에 `https://0101-commits.github.io` 를 등록(잘못된 도메인은 삭제)하면 정상 연결됩니다.
  - **네이버 증권 링크는 `go.html` 중계를 거칩니다**(2026-09-22). 카톡 알림의 사진·버튼을 네이버로
    돌리면서 생 URL 을 넣었더니, 카카오가 도메인만 등록된 것으로 갈아 끼우고 경로는 남겨
    `0101-commits.github.io/sise/sise_index.naver?code=KOSPI` → **GitHub 404** 가 떴습니다(13:16 발송본).
    이제 카카오 경로의 모든 링크는 `send_kakao_digest.kakao_link()` 를 지나 등록 도메인 위의
    `/economic-site/go.html?k=KOSPI` 로 나가고, 그 페이지가 네이버로 보냅니다. 디스코드는 이
    제약이 없어 직접 링크 그대로입니다. `go.html` 은 생성물 — `python scripts/build_go_page.py`.
...
- **`refresh_token` 만료**: `refresh_token` 은 발급 시점부터 **약 2개월 고정 만료**입니다 — 사용한다고
  ...
  `refresh_token` 을 함께 회전 발급**하고, 현재는 워크플로가 이를 `GH_SECRETS_PAT` 시크릿으로
  `KAKAO_REFRESH_TOKEN` 에 **자동 반영(auto-rotate)** 하므로 평소에는 손댈 일이 없습니다.
  자동 반영이 실패했거나(워크플로 로그의 "토큰 자동회전 실패/불가" 경고가 그 신호) 이미 만료됐다면
