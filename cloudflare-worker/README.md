...
## 📝 분석 노트 클라우드 동기화 — `GET/POST /notes`

'분석 노트' 페이지의 노트(localStorage)를 **Cloudflare KV** 에 백업/복원합니다.
다른 기기/브라우저에서 "⬇ 서버에서 불러오기"로 동일한 노트를 이어서 쓸 수 있습니다.

- `/portfolio` 와 달리 **GitHub 저장소에 커밋하지 않습니다** — 노트는 개인 분석
  내용이라 공개 저장소에 올리면 안 되므로, 비공개 KV 스토리지에만 저장합니다.
- 활성화 (1회):
  ```sh
  npx wrangler kv namespace create NOTES_KV
  # 출력된 id 를 wrangler.jsonc 의 kv_namespaces 주석 블록에 기입 후 주석 해제
  npx wrangler secret put NOTES_SYNC_KEY   # (권장) 동기화 키 — 읽기/쓰기 모두 검증
  npx wrangler deploy
  ```
- `NOTES_SYNC_KEY` 미설정 시 `ALERTS_SYNC_KEY` 를 재사용합니다. 둘 다 없으면 키 검증
  없이 동작하므로 **키 설정을 권장**합니다 (Worker URL 을 아는 누구나 읽기/쓰기 가능해짐).
- KV 바인딩이 없으면 `/notes` 는 `503 no_kv_binding` 을 반환하며, 프론트는 로컬
  저장(localStorage + 자동 백업 슬롯)으로 계속 동작합니다.
